...
export const SYSTEM = {
	...
	TAKST: 'urn:oid:2.16.578.1.12.4.1.1.8214',
	// Meldingstyper
	MELDINGSTYPE: 'urn:oid:2.16.578.1.12.4.1.1.8279',
...
export const IKKE_VERIFISERTE_SYSTEMER = new Set<string>([SYSTEM.TAKST, SYSTEM.MELDINGSTYPE, SYSTEM.NCMP]);
 ...
 */
export function gyldigNorskPersonnummer(nr: string): boolean {
...
export function gyldigDatoDel(nr: string): string | null {
	...
	const aarhundre = utledAarhundre(aarTo, individ);
	if (aarhundre === null) return null;
	const aar = aarhundre + aarTo;
	const d = new Date(Date.UTC(aar, maned - 1, dag));
	if (d.getUTCMonth() !== maned - 1 || d.getUTCDate() !== dag) return null;
	return d.toISOString().slice(0, 10);
}

function utledAarhundre(aarTo: number, individ: number): number | null {
	if (individ < 500) return 1900;
	if (individ < 750 && aarTo >= 54) return 1800;
	if (individ < 1000 && aarTo < 40) return 2000;
	if (individ >= 900 && aarTo >= 40) return 1900;
	return 1900;
}

export function erDNummer(nr: string): boolean {
	return /^\d{11}$/.test(nr) && Number(nr.slice(0, 2)) > 40;
}

/** Organisasjonsnummer: 9 siffer med mod11-kontrollsiffer. */
export function gyldigOrganisasjonsnummer(nr: string): boolean {
...
export function gyldigHprNummer(nr: string): boolean {
	return /^\d{1,9}$/.test(nr);
}

/** Kjønn utledet fra individsiffer (partall = kvinne). Kun for testdata. */
export function kjonnFraPersonnummer(nr: string): 'male' | 'female' | 'unknown' {
	if (!/^\d{11}$/.test(nr)) return 'unknown';
	return Number(nr[8]) % 2 === 0 ? 'female' : 'male';
}

export function maskerPersonnummer(nr: string): string {
