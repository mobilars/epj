...
export async function koeUt(inn: KoInn, aktor: AuditAktor): Promise<{ ok: boolean; id?: string; feil?: string }> {
	...
	const id = nyId();
	await exec(
		`INSERT INTO melding (id, retning, meldingstype, msg_id, ref_msg_id, patient_id, avsender_her, mottaker_her,
			mottaker_navn, status, payload_xml, opprettet_av, neste_forsok)
		 VALUES ($1,'ut',$2,$3,$4,$5,$6,$7,$8,'kø',$9,$10, now())`,
		[
			id, inn.meldingstype, inn.msgId, inn.refMsgId ?? null, inn.patientId,
			config.organisasjon.herId, inn.mottakerHer, mottaker?.navn ?? null, inn.payloadXml, inn.opprettetAv
		]
	);
	await logg(
		{
...
export async function sendKo(): Promise<{ sendt: number; feilet: number }> {
	const resultat = await medLaas(918_271, async () => {
		const klare = await query<{ id: string; msg_id: string; meldingstype: string; mottaker_her: string; payload_xml: string; forsok: number; patient_id: string | null }>(
			`SELECT id, msg_id, meldingstype, mottaker_her, payload_xml, forsok, patient_id FROM melding
			 WHERE retning = 'ut' AND status IN ('kø','feilet') AND (neste_forsok IS NULL OR neste_forsok <= now())
		...
		for (const m of klare) {
			try {
				...
				await exec("UPDATE melding SET status = 'sendt', oppdatert = now(), status_detalj = NULL WHERE id = $1", [m.id]);
				sendt++;
			} catch (err) {
				...
				await exec(
					`UPDATE melding SET status = $2, forsok = $3, status_detalj = $4,
					 neste_forsok = CASE WHEN $5 THEN NULL ELSE now() + $6::interval END, oppdatert = now() WHERE id = $1`,
					[m.id, oppgitt ? 'avvist' : 'feilet', forsok, melding, oppgitt, nesteForsok(forsok)]
				);
				feilet++;
			}
		}
...
export async function mottaMelding(xml: string, aktor: AuditAktor): Promise<MottakResultat> {
	...
	const duplikat = await en<{ id: string }>("SELECT id FROM melding WHERE retning = 'inn' AND msg_id = $1", [lest.msgId]);
	if (duplikat) {
		return {
			ok: true,
	...
	await transaction(async () => {
		await exec(
			`INSERT INTO melding (id, retning, meldingstype, msg_id, patient_id, avsender_her, mottaker_her,
				mottaker_navn, status, payload_xml, apprec_status)
			 VALUES ($1,'inn',$2,$3,$4,$5,$6,$7,'mottatt',$8,$9)`,
			[id, lest.type.kode, lest.msgId, patientId, lest.avsender.her, config.organisasjon.herId, lest.avsender.navn, xml, status]
		);
	});

	if (patientId) {
		const fhirRef = await speilTilFhir(lest, patientId, xml);
		if (fhirRef) await exec('UPDATE melding SET fhir_ref = $2 WHERE id = $1', [id, fhirRef]);
	}
...
export async function hentMelding(id: string): Promise<(Melding & { payload_xml: string | null }) | null> {
	return en<Melding & { payload_xml: string | null }>(`SELECT ${FELT}, payload_xml FROM melding WHERE id = $1`, [id]);
}

export async function markerBehandlet(id: string, aktor: AuditAktor): Promise<void> {
	const melding = await hentMelding(id);
	await exec("UPDATE melding SET status = 'behandlet', oppdatert = now() WHERE id = $1", [id]);
	await logg(
...
export async function ventendeKvitteringer(eldreEnnMinutter = 60): Promise<Melding[]> {
	return query<Melding>(
		`SELECT ${FELT} FROM melding
		 WHERE retning = 'ut' AND status = 'sendt' AND apprec_status IS NULL
		   AND oppdatert < now() - ($1 || ' minutes')::interval
		 ORDER BY oppdatert LIMIT 200`,
		[String(eldreEnnMinutter)]
	);
}
