...
function erPrivatAdresse(ip: string): boolean {
	if (isIP(ip) === 6) {
		...
		if (mappet) {
			const rest = mappet[1];
			if (isIP(rest) === 4) return erPrivatAdresse(rest);
			const hex = /^([0-9a-f]{1,4}):([0-9a-f]{1,4})$/.exec(rest);
			if (hex) {
				const hoy = Number.parseInt(hex[1], 16);
				const lav = Number.parseInt(hex[2], 16);
				return erPrivatAdresse(`${hoy >> 8}.${hoy & 255}.${lav >> 8}.${lav & 255}`);
			}
...
export class UtgaendeFeil extends Error {}

/** Kontrollerer at adressen er lovlig som mål for et datastyrt kall. */
export async function sjekkUtgaendeUrl(rå: string): Promise<URL> {
	let url: URL;
	try {
		url = new URL(rå);
	} catch {
		throw new UtgaendeFeil('Adressen er ikke en gyldig URL');
	}
	if (url.protocol !== 'https:') throw new UtgaendeFeil('Adressen må bruke https');
	if (url.username || url.password) throw new UtgaendeFeil('Adressen kan ikke inneholde legitimasjon');

	const vert = url.hostname.replace(/^\[|\]$/g, '');
	if (isIP(vert)) {
		if (erPrivatAdresse(vert)) throw new UtgaendeFeil('Adressen peker inn i et internt nett');
		return url;
	}
	let adresser: { address: string }[];
	try {
		adresser = await lookup(vert, { all: true });
	} catch {
		throw new UtgaendeFeil('Navnet kunne ikke slås opp');
	}
	if (adresser.length === 0) throw new UtgaendeFeil('Navnet kunne ikke slås opp');
	...
	for (const a of adresser) {
		if (erPrivatAdresse(a.address)) throw new UtgaendeFeil('Navnet peker inn i et internt nett');
	}
	return url;
}
 ...
 */
export async function hentJsonUtenfra(
	rå: string,
	{ timeoutMs = 5000, maksBytes = 512 * 1024 } = {}
): Promise<unknown> {
	const url = await sjekkUtgaendeUrl(rå);
	...
	if (svar.status >= 300 && svar.status < 400) {
		throw new UtgaendeFeil('Adressen svarte med en omdirigering, som ikke følges');
	}
	if (!svar.ok) throw new UtgaendeFeil(`Adressen svarte ${svar.status}`);
	const tekst = (await svar.text()).slice(0, maksBytes);
	try {
		return JSON.parse(tekst);
	} catch {
		throw new UtgaendeFeil('Svaret var ikke gyldig JSON');
	}
}
