...
import { loggInn, loggInnPlattform, PLATTFORM_URL, ventPaHydrering } from './hjelpere';
...
test.describe('plattformadministrasjon', () => {
	test('systemeier ser virksomhetene og partisjonstilstanden', async ({ page }) => {
		...
		await expect(tabell).toContainText('Storgata Legesenter');
		await expect(tabell).toContainText('Plattformadministrasjon');
		// Plattformvirksomheten har ingen FHIR-partisjon, og skal ikke meldes som avvik.
		await expect(tabell).not.toContainText('mangler i HAPI');
	});

	test('oppretter en virksomhet med partisjon og første systemansvarlige', async ({ page }) => {
		await loggInnPlattform(page);
		await page.goto(`${PLATTFORM_URL}/systemadmin`);
		...
		const kvittering = page.getByRole('status').filter({ hasText: 'legekontor-nytt' });
		await expect(kvittering).toContainText('er opprettet med egen FHIR-partisjon');
		// Det midlertidige passordet vises én gang, og bare her.
		await expect(kvittering).toContainText('sjefen');

		// Virksomheten er nå i oversikten, med en partisjon som finnes i HAPI.
		const rad = page.getByRole('row').filter({ hasText: 'Nytt Legekontor AS' });
		await expect(rad).toContainText('nytt.epj.test');
		await expect(rad).toContainText('Aktiv');
		await expect(rad).not.toContainText('mangler i HAPI');
	...
	test('suspensjon stenger virksomheten umiddelbart', async ({ page }) => {
		...
		const rad = page.getByRole('row').filter({ hasText: 'Storgata Legesenter' });
		await rad.getByRole('button', { name: 'Suspender' }).click();
	...
	test('plattformvirksomheten kan ikke suspenderes', async ({ page }) => {
		await loggInnPlattform(page);
		const svar = await page.request.post(`${PLATTFORM_URL}/systemadmin?/status`, {
			form: { id: 'plattform', status: 'suspendert' }
		});
		expect(await svar.text()).toContain('kan ikke suspenderes');
	});
	...
	test('plattformadministrasjonen finnes ikke på en virksomhets vertsnavn', async ({ page }) => {
		...
		const paaVirksomhet = await page.request.get('/systemadmin');
		expect(paaVirksomhet.status()).toBe(404);
		expect(await paaVirksomhet.text()).toContain('eget vertsnavn');
	});
	...
	test('systemeier har ingen klinisk tilgang', async ({ page }) => {
		...
		const pasienter = await page.request.get(`${PLATTFORM_URL}/pasienter`);
		expect(pasienter.status()).toBe(404);

		// Og rollen har ikke ett eneste scope, så FHIR-fasaden gir ingenting.
		const fhir = await page.request.get(`${PLATTFORM_URL}/fhir/Patient`);
